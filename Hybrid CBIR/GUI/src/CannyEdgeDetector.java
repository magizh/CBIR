...
public class CannyEdgeDetector {
  ...
  /*	public static void main(String[] args) {
		...
		//Create 3-D Matrix for image pixels
		int pix[][][] = new int [50][128][128];

		String dir = "C:/Users/Magi/cannytest/img"; // Location of image directory
		...
		for (int z = 0;z < filename.length;z++) {
			int flag = 1;
			ImageIcon icon = new ImageIcon(dir+"/"+filename[z]);
			...
			// Create empty BufferedImage, sized to Image
                	...
                	//adjust its parameters as desired
	                //detector.setLowThreshold(0.5f);
        	        //detector.setHighThreshold(1.0f);//apply it to an image
			...
			//Convert edge-detected BufferedImage to Grayscale
			BufferedImage imgs =
        	                new BufferedImage(
	                                edges.getWidth(),
        	                        edges.getHeight(),
			...
			Graphics h = imgs.getGraphics();
                	h.drawImage(edges, 0, 0, null);
			//h.dispose();
			//Image imgss = imgs;

			//Convert Image to 128x128 matrix
			DataBuffer dataBuffer = imgs.getRaster().getDataBuffer();
	           	int c = 0;
			for(int i = 0;i < 128;i++) {
				for(int j = 0;j < 128;j++) {
					pix[z][i][j] = dataBuffer.getElem(j * imgs.getWidth() + i);
					//System.out.print(" "+pix[i][j]);
				}
			}

			// Discrete-Wavelet Transformation transforms 128x128 pixels to 32x32 matrix
			int x,y,i,j,rows = 128,cols = 128;
			while(flag == 1) {
    				if(rows == 32)
         				break;
	    			for(x = 0;x <rows;x++) {
					...
					int temp[] = new int[cols];
	        			while(flag == 1) {
            					if(y == cols)
             						break;
	            				temp[i] = (pix[z][x][y]+pix[z][x][y+1])/2;
        	    				temp[i+(cols/2)] = (pix[z][x][y]-pix[z][x][y+1])/2;
            					i += 1;
            					y += 2;
					}
					//System.out.println(" ");
				...
				System.out.println("--------------------------------------------------------------------------------- ");
					...
					int temp[] = new int[rows];
						...
						if(y == rows)
	             					break;
        	    				temp[i] = (pix[z][y][x]+pix[z][y+1][x])/2;
          					temp[i+(rows/2)] = (pix[z][y][x]-pix[z][y+1][x])/2;
	         				i += 1;
        	    				y += 2;
	        			}
					//System.out.println(" ");
			...
			int pixi[] = new int[32*32];
			for(int f = 0; f < 32; f++) {
				//System.out.println(" ");
				for(int w = 0, k = 0; w < 32; w++, k++) {
					pixi[k]=pix[z][f][w];
					//System.out.print(" "+pix[z][f][w]);
				}
			}
			...
			//for(i=0;i<=128;i++){
        		raster.setPixels(0, 0, 32, 32, pixi);//}
				//Graphics t = img.getGraphics();
			...
			Graphics t = imgss.getGraphics();
            t.drawImage(img, 0, 0, null);
			img.getWritableTile(0, 0).setDataElements(0, 0, 32, 32, pixi);
			/*BufferedImage imgss =
        	                new BufferedImage(
	                                img.getWidth(),
        	                        img.getHeight(),
